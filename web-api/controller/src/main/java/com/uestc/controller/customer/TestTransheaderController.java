package com.uestc.controller.customer;

import com.uestc.common.Result;
import com.uestc.service.TestTransheaderService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.ResponseBody;

/**
 * Created by fucheng on 2019/7/15.
 */

@Controller
@RequestMapping(value = "api")
public class TestTransheaderController {

    @Autowired
    private TestTransheaderService testTransheaderService;

    @RequestMapping(value = "transheader/list",method = RequestMethod.GET)
    @ResponseBody
    public Result getTestTransheaderList(){

        return testTransheaderService.getTransheaderList();
    }
}
